...
- Redirect URIs: exact match after URL normalization (RFC 6749 §3.1.2.3); only `http`-scheme loopback URIs (`127.0.0.1`, `[::1]`, plus `localhost` unless `treatLocalhostAsLoopback: false`) may vary the port (RFC 8252 §7.3); explicit `redirect_uri` required unless exactly one URI is registered (see `docs/adr/0007`)
