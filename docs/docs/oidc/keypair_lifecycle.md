...
The first OIDC release uses **one key**: one active RSA key, and one entry in the JWKS. Thus you change the key in one step. The server signs with the new key, and each token from the old key becomes invalid when you remove the old key.
