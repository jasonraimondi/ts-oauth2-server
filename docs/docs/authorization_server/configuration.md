...
| `implicitRedirectMode`   | "query" \| "fragment" | "query" | Controls how the [implicit grant](../grants/implicit.mdx) appends tokens to the redirect URI. OAuth 2.0 ([RFC 6749 §4.2.2](https://datatracker.ietf.org/doc/html/rfc6749#section-4.2.2)) recommends `"fragment"`; `"query"` is the default for backwards compatibility. <br /><br />In the next major version, the default will change to **"fragment"**. |
