...
- At authorization time, the requested `redirect_uri` must exactly match a registered URI — host, path, port, and query string all included ([RFC 6749 §3.1.2.3](https://datatracker.ietf.org/doc/html/rfc6749#section-3.1.2.3)). Only loopback hosts (`localhost`, `127.0.0.1`, `[::1]`) may vary the port ([RFC 8252 §7.3](https://datatracker.ietf.org/doc/html/rfc8252#section-7.3)), so register every variant you redirect to.
