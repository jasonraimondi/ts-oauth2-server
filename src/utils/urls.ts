 ...
 * registered URI's host is loopback, the port may vary between registration
 * and request — the host itself must still match exactly.
 */
export function redirectUriMatches(requested: string, registered: string): boolean {
  ...
  if (LOOPBACK_HOSTNAMES.has(registeredUrl.hostname) && requestedUrl.hostname === registeredUrl.hostname) {
    requestedUrl.port = "";
    registeredUrl.port = "";
  }
